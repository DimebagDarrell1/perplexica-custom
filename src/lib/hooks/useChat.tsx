      ...
      msg.responseBlocks.forEach((block) => {
        if (block.type === 'text') {
...

          {
            ...
            // Has open tag but no close tag yet (still streaming)
            if (openThinkTag && !closeThinkTag) {
              processedText += '</think> <a> </a>';
            }

            // deepseek-r1-671b emits </think> without an opening <think> tag.
            // Prepend a synthetic open tag so the ThinkBox renders correctly.
            if (!openThinkTag && closeThinkTag) {
